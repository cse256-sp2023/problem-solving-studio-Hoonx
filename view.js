...
let view_permissions = define_new_effective_permissions(
  'perm_',
  (add_info_col = true),
  (which_permissions = null)
);
$('#sidepanel').append(view_permissions);

let select_user = define_new_user_select_field(
  'usr_',
  'User Selection',
...
let explanationDialog = define_new_dialog('explanationDialog', 'Explanation', {
  width: 400,
  height: 200,
  buttons: {
    Close: function () {
...
$('.perm_info').click(function () {
  ...
  const filepath = $('#perm_').attr('filepath');
  const username = $('#perm_').attr('username');
  const permissionType = $(this).attr('permission_name');
  const fileObject = path_to_file[filepath];
  const userObject = all_users[username];

  const explanation = allow_user_action(
    fileObject,
    userObject,
    permissionType,
    true
  ...
  $('#explanationDialog').empty().html(explanationText);
});
